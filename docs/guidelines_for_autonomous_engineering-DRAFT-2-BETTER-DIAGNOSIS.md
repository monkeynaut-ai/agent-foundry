...
1. **LLM self-governance degrades where quality gradients are shallow.** LLMs can produce specs, tests, code, and plans quickly, and frontier models follow explicit instructions reliably in well-scoped tasks. But the reliability of self-governance degrades as the difference between a good artifact and a subtly weak one becomes harder to distinguish from the output alone. "Produce a high-quality specification" is underdetermined in ways that "follow this formatting rule" is not, and models resolve that underdetermination toward least resistance at a rate that compounds across an autonomous pipeline. The failure mode is not crude rule-breaking; it is subtle drift under pressure—slightly vague assertions, loosely defined edge cases, specifications that technically address every requirement but leave room for easy implementation rather than correct implementation. This drift is correlated: it tends to occur on the hard cases where correctness matters most. Structural controls are therefore not compensating for models that cannot follow instructions; they are compensating for a domain in which "follow these quality standards" is an insufficient instruction even for capable models.
...
In summary: **autonomous software engineering is a control problem under uncertainty, not just a generation problem**. The core difficulty is creating a system in which LLM-based agents learn the right things, apply pressure to the right risks, and cannot easily convert apparent progress into false progress. This control problem persists even as model capability improves, because the characteristics that make self-governance insufficient—shallow quality gradients, judgment-dependent artifacts, and underdetermined quality standards—are properties of the domain, not just limitations of current models.

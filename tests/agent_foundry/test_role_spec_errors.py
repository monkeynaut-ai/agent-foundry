...
Tests: missing required fields yield RoleSpecValidationError;
...
class TestMissingRequiredFields:
    ...
    def test_missing_name_raises_validation_error(self):
        with pytest.raises(RoleSpecValidationError) as exc_info:
            load_role_spec(FIXTURES / "invalid_missing_name.yaml")
...
class TestParseErrors:
    ...
    def test_invalid_yaml_raises_parse_error(self):
        with pytest.raises(RoleSpecParseError) as exc_info:
            load_role_spec(FIXTURES / "invalid_parse_error.yaml")
        assert exc_info.value.file_path == FIXTURES / "invalid_parse_error.yaml"

    def test_invalid_json_raises_parse_error(self):
        with pytest.raises(RoleSpecParseError) as exc_info:
            load_role_spec(FIXTURES / "invalid_parse_error.json")
    ...
    def test_unsupported_extension_raises_parse_error(self, tmp_path):
        ...
        with pytest.raises(RoleSpecParseError, match="Unsupported file extension"):
            load_role_spec(path)

    def test_yaml_non_object_raises_parse_error(self, tmp_path):
        path = tmp_path / "capability.yaml"
        path.write_text("- just\n- a\n- list\n")
        with pytest.raises(RoleSpecParseError, match="Expected a mapping"):
            load_role_spec(path)
...
class TestEdgeCases:
    def test_nonexistent_file_raises_parse_error(self):
        with pytest.raises(RoleSpecParseError):
            load_role_spec(FIXTURES / "does_not_exist.yaml")

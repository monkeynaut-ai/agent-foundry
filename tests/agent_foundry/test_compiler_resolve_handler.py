...
class TestResolveHandler:
    def test_given_capability_in_handler_registry_then_returns_that_handler(self):
        ...
        result = _resolve_handler(
            "n1", "test_cap", {"test_cap": handler}, CapabilityRegistry(specs={})
        )
    ...
    def test_given_non_callable_in_handler_registry_then_raises_instantiation_error(self):
        with pytest.raises(CapabilityInstantiationError) as exc_info:
            _resolve_handler(
                "n1", "test_cap", {"test_cap": "not_callable"}, CapabilityRegistry(specs={})
            )
        assert exc_info.value.node_id == "n1"

    def test_given_capability_not_in_handler_registry_but_valid_in_registry_then_dynamic_import(
        self,
    ):
    ...
    def test_given_capability_not_in_handler_registry_and_import_fails_then_raises_instantiation_error(
        self,
    ):
